 ...
 ***********************/
/***********************
 * SOLO CHALLENGES (TEMP - replace later)
 ***********************/
const SOLO_CHALLENGES = [
  { id: "solo_pushups",   title: "Solo Challenge", desc: "Do 10 push-ups. Stop = fail." },
  { id: "solo_no_smile",  title: "Solo Challenge", desc: "Hold a straight face for 20 seconds while everyone tries to make you laugh." },
  { id: "solo_one_leg",   title: "Solo Challenge", desc: "Stand on one leg for 30 seconds. Put your foot down = fail." },
];

const EVENT_DEFS = [
  { id: "SOLO_CHALLENGE",  weight: 3, start: (ctx) => startSoloChallenge(ctx) },
...
function drawSoloChallenge() {
  // Simple random draw (you can replace with a non-repeating bag later if you want).
  return SOLO_CHALLENGES[Math.floor(Math.random() * SOLO_CHALLENGES.length)];
}

function startSoloChallenge(ctx) {
  const ch = drawSoloChallenge();

  const targetName = ctx.target?.name ?? "Player";
  ctx.setTitle(ch.title);
  ctx.setBody(`${targetName}: ${ch.desc}`);

  // Build actions: Fail / Complete
  eventActions.innerHTML = "";

  const failBtn = document.createElement("button");
  failBtn.textContent = "Fail";
  failBtn.addEventListener("click", () => {
    // No points, just go to next event (which will also pick a new target fairly).
    startNewEvent();
  });

  const completeBtn = document.createElement("button");
  completeBtn.className = "primary";
  completeBtn.textContent = "Complete (+1)";
  completeBtn.addEventListener("click", () => {
    if (ctx.target) ctx.target.points += 1;
    startNewEvent();
  });

  eventActions.appendChild(failBtn);
